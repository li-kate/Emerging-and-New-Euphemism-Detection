...
def load_second_pass(path: Path, max_rows: int | None = None) -> pd.DataFrame:
    rows = []
...
def main() -> int:
    ...
    df_raw = load_second_pass(second_pass_jsonl, max_rows=None)
    ...
    if not args.no_plot and task_id == 0:
        ...
        period_order = sorted(wide.index, key=lambda x: period_sort_key(str(x)))
        wide = wide.loc[period_order]
